    ...
    void DrawDynamicInspector(Material material, MaterialEditor materialEditor, MaterialProperty[] props)
    {
            ...
            for (int i = 0; i < props.Length; i++)
            {
                ...
                if (!showGlobalsCat)
                {
                    if (prop.name == "_GlobalsCat")
                        continue;
                    if (prop.name == "_SpaceGlobalPosition")
                        continue;
                }
                ...
                {
                    if (material.GetFloat("_GlobalAlpha") == 0 && prop.name == "_AlphaMaskRemap")
                        continue;
                ...
                {
                    if (material.GetFloat("_LeavesFilterMode") == 0 && prop.name == "_LeavesFilterColor")
                        continue;

                    if (material.GetFloat("_LeavesFilterMode") == 0 && prop.name == "_LeavesFilterRange")
                        continue;
                }

                if (!material.HasProperty("_LeavesFilterMode"))
                {
                    if (prop.name == "_MessageLeavesFilter")
                        continue;
                }
                ...
                if (shaderName.Contains("Translucency"))
                {
                    ...
                    if (material.GetTag("RenderPipeline", false) != "HDRenderPipeline")
                    {
                        if (prop.name == "_MessageTranslucencyHD")
                            continue;
                    }
                }
                else
                ...
                {
                    if (prop.name == "_SpaceMotionHighlight")
                        continue;
                }
                ...
                {
                    if (material.GetInt("_VertexDataMode") == 1 && prop.name == "_ColorsPositionMode")
                        continue;

                    if (material.GetInt("_VertexDataMode") == 1 && prop.name == "_ExtrasPositionMode")
                        continue;

                    if (material.GetInt("_VertexDataMode") == 1 && prop.name == "_SpaceGlobalPosition")
                        continue;
        ...
        for (int i = 0; i < customPropsList.Count; i++)
        {
            ...
            if (EditorGUIUtility.currentViewWidth > 500)
            {
                ...
                {
                    if (!shaderName.Contains("Simple Lit") && !shaderName.Contains("Polygonal"))
                    {
                        if (shaderName.Contains("Grass"))
                        {
                            displayName = "Color Mask (Bottom Mask)";
                        }

                        if (shaderName.Contains("Leaf"))
                        {
                            displayName = "Color Mask (Mask Blue)";
                        }
